...
export function matchTransaction(complaint, history) {
    ...
    // if day hint given and we still have multiple, narrow by recency using the most recent txn as "today" anchor
    ...
    // multiple candidates: check duplicate-payment pattern (same amount, same counterparty, close timestamps)
    ...
    // ambiguous: multiple plausible matches, different counterparties -> don't guess
    return { match: null, candidates, verdict: "insufficient_data", ambiguous: true };
}

// Detect contradiction: e.g. "wrong transfer" claim but multiple prior transfers to same counterparty
export function checkInconsistency(caseType, match, history) {
    ...
    // Customer claims payment failed, but transaction shows completed → contradiction
    if (caseType === "payment_failed" && match) {
        if (match.status === "completed") return "inconsistent";
    }

    // Customer requests refund, but transaction is already reversed → already refunded
