/**
 * test_http_status_codes.js
 *
 * Verifies that every required HTTP status code is correctly returned
 * by the QueueStorm /analyze-ticket endpoint, and that:
 *   - 200 bodies conform to the output schema
 *   - 400/422/500 bodies carry a non-sensitive error message
 *   - 500 does NOT expose stack traces, tokens, or secrets
 *
 * Run AFTER the server is already started:
 *   npm start           (terminal 1)
 *   node test/test_http_status_codes.js   (terminal 2)
 */
...
const VALID_BODY = {
    ...
    language: "en",
    user_type: "customer",       // valid enum: customer|merchant|agent|unknown
    transaction_history: []
};
